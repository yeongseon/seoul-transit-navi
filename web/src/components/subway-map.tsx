...
export function SubwayMap() {
  ...
  const [lineStatsExpanded, setLineStatsExpanded] = useState(false);
  ...
  const totalStationCount = useMemo(() => new Set(SUBWAY_MAP_LINES.flatMap((line) => line.stationIds)).size, []);
      ...
      <div className="relative lg:grid lg:grid-cols-[1fr_380px] lg:items-start lg:gap-4">
        <div className="space-y-4">
          <section className="overflow-hidden rounded-[28px] bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div
              ref={scrollRef}
              className="min-h-[60vh] overflow-auto bg-[radial-gradient(circle_at_center,rgba(15,23,42,0.04)_0,rgba(15,23,42,0.04)_1px,transparent_1px)] [background-position:0_0] [background-size:20px_20px]"
            >
          ...
          <details
            className="overflow-hidden rounded-[28px] bg-white/95 shadow-sm ring-1 ring-slate-200"
            open={legendExpanded}
          ...
          >
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-sm font-semibold text-slate-900 marker:hidden">
              <span>{legendExpanded ? t("subwayMap.hideLegend") : t("subwayMap.showLegend")}</span>
              <span
                aria-hidden="true"
                className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-600 transition-transform ${legendExpanded ? "rotate-180" : "rotate-0"}`}
              >
                ↓
              </span>
            </summary>
          ...
          <details
            className="overflow-hidden rounded-[28px] bg-white/95 shadow-sm ring-1 ring-slate-200"
            open={lineStatsExpanded}
            onToggle={(event) => setLineStatsExpanded(event.currentTarget.open)}
          >
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-sm font-semibold text-slate-900 marker:hidden">
              <div>
                <p>{t("subwayMap.lineStats")}</p>
                <p className="mt-1 text-xs font-medium text-slate-500">{t("subwayMap.stationCount", { count: totalStationCount })}</p>
              </div>
              <span
                aria-hidden="true"
                className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-600 transition-transform ${lineStatsExpanded ? "rotate-180" : "rotate-0"}`}
              >
                ↓
              </span>
            </summary>
            ...
            <div className="border-t border-slate-200/80 px-5 py-4">
              <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                {SUBWAY_MAP_LINES.map((line) => (
                  <div key={line.id} className="rounded-2xl bg-slate-50 px-4 py-3 ring-1 ring-slate-200">
                    <div className="flex items-center gap-3">
                      <span className="h-3 w-8 rounded-full" style={{ backgroundColor: line.color }} />
                      <p className="text-sm font-semibold text-slate-900">{locale === "ko" ? line.nameKo : line.nameJa}</p>
                    </div>
                    <p className="mt-2 text-xs text-slate-500">{t("subwayMap.stationCount", { count: line.stationIds.length })}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="pointer-events-none fixed inset-x-0 bottom-0 z-20 px-4 pb-4 lg:pointer-events-auto lg:sticky lg:top-4 lg:z-auto lg:px-0 lg:pb-0">
          ...
          >
            ...
            <section id="subway-map-info-panel" aria-label="Station information" className="rounded-b-[28px] lg:rounded-[28px]">
